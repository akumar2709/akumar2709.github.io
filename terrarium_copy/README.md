...
- Starts the MCP server connection at `http://localhost:8000/mcp` (ensure `python src/server.py` is running).
- Replaces each agent with the appropriate subclass (`AgentPoisoningAttack` or `ContextOverflowAttack`).
- Logs attack metadata to the standard log directory alongside tool calls and blackboard state.

### Running protocol poisoning

Protocol poisoning operates at the communication layer rather than per-agent:
...
1. Boots the environment as usual.
2. Calls `CommunicationProtocolPoisoningAttack.inject(...)` before each phase to seed every active blackboard with the payload defined in `examples/configs/attack_config.yaml`.
3. Records injected payload statistics to `attack_events.jsonl` / `attack_summary.json` for later analysis (used by the dashboard).

> Tip: Copy or edit `examples/configs/attack_config.yaml` and modify the keys listed in the table above to craft richer attacks without touching source code.
...
The `/dashboards` directory contains a static React UI that visualizes simulation logs and attack telemetry without running a backend service.

### What you get

- **Run directory browser** – cards for every `logs/<env>/<tag_model>/<run_timestamp>/seed_<n>/` folder with seed, score history, and attack counts.
- **Attack analytics** – stacked success/failure chart built from `attack_summary.json` across all runs.
- **Inline log viewers** – quick access to blackboard transcripts, tool calls, agent prompts (JSON/Markdown), trajectories, and raw event streams.
- **Config snapshot** – the YAML config embedded in the bundle appears in the header for easy provenance tracking.
...
- `--logs-root` defaults to `logs/`; point it at any run directory.
- `--config` is optional but recommended so the UI shows which scenario produced the data.
- `--output` controls where the JSON bundle is written (the default matches the shipping front-end).
...
Then visit <http://127.0.0.1:5050> (or open `index.html` directly if your browser permits `file://` fetch requests). To refresh with new runs, re-run the build step and reload the page—no need to restart the server.

Customization starts from `dashboards/templates/index.html`; rebuild the static `public/index.html` if you tweak layout or styling.
